...
class net(nn.Module):
    def __init__(self, bn=False):
        ...
        self.adaptive_encoder = nn.Sequential(nn.Conv2d(1, 128, kernel_size=3, padding=1),
                                              nn.ReLU(inplace=True),
                                              nn.MaxPool2d(2),
        ...
        self.adaptive_decoder = nn.Sequential(nn.ConvTranspose2d(256, 128, kernel_size=2, stride=2),
                                              nn.ReLU(inplace=True),
    ...
    def forward(self, image, mini_image, point_map):
        ...
        adaptive_encoder_feature = self.adaptive_encoder(point_map)
        adaptive_decoder_feature = self.adaptive_decoder(adaptive_encoder_feature)
        adaptive_density_map = self.adaptive_output_layer(adaptive_decoder_feature)

        return predicted_density_map, adaptive_density_map
